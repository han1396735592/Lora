...
extern u8* model_select;
extern _MYDATA _mydata;
//主测试函数
void lora_app_run(void)
{	
	u8 key=0;
	u8 netpro=0;
	_mydata.model=0;
	...
	{		
		...
		Show_Str(40+110+10,45+80,200,16,model,16,0);
		key = KEY_Scan(0);
		if(key)
		{
			Show_Str(30+10,80+45+netpro*25,200,16,"  ",16,0);//清空之前的显示
			...
			}else if(key==WKUP_PRES)//KEY_UP按下
			{
				...
				if(netpro==7){  // 进入状态监控
				  dashboard_process(); //显示状态监控
				  netpro=0;//索引返回第0
				  Menu_ui();
				  
				}else{
					Show_Str(30+40,80+45+netpro*25+2,200,16,"________",16,1);//显示下划线,表示选中
					Show_Str(30+10,80+45+netpro*25,200,16,"→",16,0);//指向新条目
					Menu_cfg(netpro);//参数配置
					LCD_Fill(30+40,80+45+netpro*25+2+15,30+40+100,80+45+netpro*25+2+18,WHITE);//清除下划线显示
				}
